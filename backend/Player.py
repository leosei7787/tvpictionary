...
class PlayerProperty(db.Property):
    ...
    def get_value_for_datastore(self, model_instance):
        ...
        if (player == None):
            logging.info(player)
            return None
        return json.dumps({"player_name": player.player_name, "key_name": player.key_name})

    # For reading from datastore.
    def make_value_from_datastore(self, value):
        if value is None:
            logging.info("RATILLAAAAAAAA")
            return None
        data = json.loads(value)
        return Player(key_name = data.key_name, player_name = data.player_name)
